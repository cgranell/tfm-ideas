...
A type of visualisation is small multiples. A student will work on an improved version of small multiples (coloured, annotations, interactive) as a way to visually compare routes over the past weeks, and so to determine the variability of the routes taken by patients. Small multiples might take the form of a matrix, where each row refers to a patient, and each column refers to a day. As a result, each cell would contain a representation of the trajectory/route, with the aim to make explicit how diverse trajectories are compared to time and patients. How might small multiples be classified (and coloured) based on the index of diversity? Other questions in collaboration with the users of these novel visualisation designs will surely emerge during the project. 
...
*Related work*: Literature on mobility, movement analysis, etc.
